...
def get_cuda_device_specs() -> List[Dict[str, Any]]:
    ...
    if result != CUDA_SUCCESS:
        ...
        raise RuntimeError(
            f'cuInit failed with error code {result}: {error_str.value.decode()}'
        )
    result = cuda.cuDeviceGetCount(ctypes.byref(num_gpus))
    if result != CUDA_SUCCESS:
        cuda.cuGetErrorString(result, ctypes.byref(error_str))
        raise RuntimeError(
            f'cuDeviceGetCount failed with error code {result}: {error_str.value.decode()}'
        )
    ...
    for i in range(num_gpus.value):
        ...
        if result != CUDA_SUCCESS:
            cuda.cuGetErrorString(result, ctypes.byref(error_str))
            raise RuntimeError(
                f'cuDeviceGet failed with error code {result}: {error_str.value.decode()}'
            )
        ...
        if cuda.cuDeviceComputeCapability(ctypes.byref(cc_major),
            ...
            spec.update(
                architecture=SEMVER_TO_ARCH.get((cc_major.value,
                                                 cc_minor.value), 'unknown'))
        if cuda.cuDeviceGetAttribute(ctypes.byref(cores),
                                     CU_DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT,
                                     device) == CUDA_SUCCESS:
            spec.update(cores=cores.value,
                        cuda_cores=cores.value * SEMVER_TO_CORES.get(
                            (cc_major.value, cc_minor.value), 'unknown'))
            if cuda.cuDeviceGetAttribute(
                    ctypes.byref(threads_per_core),
                    CU_DEVICE_ATTRIBUTE_MAX_THREADS_PER_MULTIPROCESSOR,
                    device) == CUDA_SUCCESS:
                spec.update(concurrent_threads=cores.value *
        ...
        if result != CUDA_SUCCESS:
            cuda.cuGetErrorString(result, ctypes.byref(error_str))
            warn(
                f'cuCtxCreate failed with error code {result}: {error_str.value.decode()}'
            )
        else:
            try:
                result = cuda.cuMemGetInfo_v2(ctypes.byref(free_mem),
                                              ctypes.byref(total_mem))
            except AttributeError:
                result = cuda.cuMemGetInfo(ctypes.byref(free_mem),
                                           ctypes.byref(total_mem))
            if result == CUDA_SUCCESS:
                spec.update(total_mem_mb=total_mem.value / 1024**2,
                            free_mem_mb=free_mem.value / 1024**2)
            else:
                cuda.cuGetErrorString(result, ctypes.byref(error_str))
                warn(
                    f'cuMemGetInfo failed with error code {result}: {error_str.value.decode()}'
                )
...
if __name__ == '__main__':
    ...
    if len(gpu_properties) > 0:
        print("".join(
            [str(i) for i in gpu_properties[0]["compute_capability"]]))
    else:
        print("UNKNOWN")
