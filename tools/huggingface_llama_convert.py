...
def split_and_convert(args):
    ...
    for l in range(num_layers):
        ...
        if kv_head_rep_num == 1:
            ...
            q_weight = param_to_weights(model.state_dict()[f'model.layers.{l}.self_attn.q_proj.weight']).reshape(factor, head_num // factor * head_size, hidden_size)
            # [kv_head_num * head_size, hidden_size]->[factor, kv_head_num // factor * head_size, hidden_size]
            k_weight = param_to_weights(model.state_dict()[f'model.layers.{l}.self_attn.k_proj.weight']).reshape(factor, kv_head_num // factor * head_size, hidden_size)
            # [kv_head_num * head_size, hidden_size]->[factor, kv_head_num // factor * head_size, hidden_size]
            v_weight = param_to_weights(model.state_dict()[f'model.layers.{l}.self_attn.v_proj.weight']).reshape(factor, kv_head_num // factor * head_size, hidden_size)
            # [factor, head_num // factor * head_size + 2 * kv_head_num // factor * head_size, hidden_size]
            qkv_weights = np.concatenate((q_weight, k_weight, v_weight), axis=1)
            # [hidden_size, factor * (head_num // factor * head_size + 2 * kv_head_num // factor * head_size)]
            qkv_weights = np.transpose(qkv_weights, (2, 0, 1)).reshape(hidden_size, -1)
