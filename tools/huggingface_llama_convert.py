...
def split_and_convert(args):
    ...
    try:
        ...
        if "rope_theta" in hf_config:
            ...
            config['llama']["rope_theta"] = str(10000.0)
        ...
        config['llama']["max_position_embeddings"] = str(
            hf_config["max_position_embeddings"])

        with open((Path(saved_dir) / f"config.ini").as_posix(),
                  'w') as configfile:
    ...
    param_to_weights = lambda param: param.detach().cpu().numpy().astype(
        np_weight_data_type)
    ...
    for l in range(num_layers):
        ...
        if kv_head_rep_num == 1:
            ...
            q_weight = param_to_weights(
                model.state_dict()
                [f'model.layers.{l}.self_attn.q_proj.weight']).reshape(
            ...
            k_weight = param_to_weights(
                model.state_dict()
                [f'model.layers.{l}.self_attn.k_proj.weight']).reshape(
                    factor, kv_head_num // factor * head_size, hidden_size)
            # [kv_head_num * head_size, hidden_size]->[factor, kv_head_num // factor * head_size, hidden_size]
            v_weight = param_to_weights(
                model.state_dict()
                [f'model.layers.{l}.self_attn.v_proj.weight']).reshape(
                    factor, kv_head_num // factor * head_size, hidden_size)
            # [factor, head_num // factor * head_size + 2 * kv_head_num // factor * head_size, hidden_size]
            qkv_weights = np.concatenate((q_weight, k_weight, v_weight),
                                         axis=1)
            # [hidden_size, factor * (head_num // factor * head_size + 2 * kv_head_num // factor * head_size)]
            qkv_weights = np.transpose(qkv_weights,
                                       (2, 0, 1)).reshape(hidden_size, -1)
        qkv_weights_base_name = f'model.layers.{l}.attention.query_key_value.weight'
        split_and_convert_process(saved_dir, factor, qkv_weights_base_name,
                                  qkv_weights)

        # attention dense
        o_weight = param_to_weights(
            model.state_dict()[f'model.layers.{l}.self_attn.o_proj.weight']).T
        o_weight_base_name = f'model.layers.{l}.attention.dense.weight'
        split_and_convert_process(saved_dir, factor, o_weight_base_name,
                                  o_weight)

        # MLP
        mlp_down_weight = param_to_weights(
            model.state_dict()[f'model.layers.{l}.mlp.down_proj.weight']).T
        mlp_down_base_name = f'model.layers.{l}.mlp.down_proj.weight'
        split_and_convert_process(saved_dir, factor, mlp_down_base_name,
                                  mlp_down_weight)

        mlp_gate_weight = param_to_weights(
            model.state_dict()[f'model.layers.{l}.mlp.gate_proj.weight']).T
        mlp_gate_base_name = f'model.layers.{l}.mlp.gate_proj.weight'
        split_and_convert_process(saved_dir, factor, mlp_gate_base_name,
                                  mlp_gate_weight)

        mlp_up_weight = param_to_weights(
            model.state_dict()[f'model.layers.{l}.mlp.up_proj.weight']).T
        mlp_up_base_name = f'model.layers.{l}.mlp.up_proj.weight'
        split_and_convert_process(saved_dir, factor, mlp_up_base_name,
                                  mlp_up_weight)

        # LayerNorm
        input_ln_weight = param_to_weights(
            model.state_dict()[f'model.layers.{l}.input_layernorm.weight'])
        input_ln_base_name = f'model.layers.{l}.input_layernorm.weight'
        split_and_convert_process(saved_dir, factor, input_ln_base_name,
                                  input_ln_weight)

        post_attn_ln_weight = param_to_weights(
            model.state_dict()
            [f'model.layers.{l}.post_attention_layernorm.weight'])
        post_attn_ln_base_name = f'model.layers.{l}.post_attention_layernorm.weight'
        split_and_convert_process(saved_dir, factor, post_attn_ln_base_name,
                                  post_attn_ln_weight)
    ...
    for name, param in model.named_parameters():
        if name == 'model.embed_tokens.weight':
            param.detach().cpu().numpy().astype(np_weight_data_type).tofile(
                saved_dir + "model.wte.weight.bin")
