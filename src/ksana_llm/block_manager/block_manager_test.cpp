...
// 定义一个 BlockManagerNvidiaTest 类，继承自 testing::Test
class BlockManagerNvidiaTest : public testing::Test {
 protected:
  // 在每个测试用例执行之前调用的函数
  void SetUp() override {
...
// 定义一个测试用例，继承自 BlockManagerNvidiaTest
TEST_F(BlockManagerNvidiaTest, AllocateAndFree) {
  // 创建一个整数向量，用于存储分配的内存块
...
TEST_F(BlockManagerNvidiaTest, AllocateAndFreeContiguousMemory) {
  int block_id;
  int64_t size = 1024;
...
TEST_F(BlockManagerNvidiaTest, SwapInAndSwapOut) {
  ...
  CUDA_CHECK(cudaMemcpy(addrs[0], string_a.data(), string_a.size(), cudaMemcpyHostToDevice));
  CUDA_CHECK(cudaMemcpy(addrs[1], string_b.data(), string_b.size(), cudaMemcpyHostToDevice));
  ...
  CUDA_CHECK(cudaMemcpy(string_a.data(), addrs[0], string_a.size(), cudaMemcpyDeviceToHost));
  CUDA_CHECK(cudaMemcpy(string_b.data(), addrs[1], string_b.size(), cudaMemcpyDeviceToHost));
...
// 定义一个测试用例，继承自 BlockManagerNvidiaTest
TEST_F(BlockManagerNvidiaTest, GetFreeBlockNumber) {
...
// 定义一个 BlockManagerNvidiaTest 类，继承自 testing::Test
class BlockManagerAscendTest : public testing::Test {
 protected:
  // 在每个测试用例执行之前调用的函数
  void SetUp() override {
    // 创建一个 BlockManagerConfig 对象，用于配置 BlockManager
    ...
    block_manager_config.device_allocator_config.blocks_num = 2;
    block_manager_config.device_allocator_config.block_size = 1024;
    block_manager_config.device_allocator_config.device = MEMORY_ASCEND;
...
// 定义一个测试用例，继承自 BlockManagerAscendTest
TEST_F(BlockManagerAscendTest, AllocateAndFree) {
  // 创建一个整数向量，用于存储分配的内存块
  std::vector<int> blocks;
...
// 测试 BlockManager 类的 AllocateAndFreeContiguousMemory 方法
TEST_F(BlockManagerAscendTest, AllocateAndFreeContiguousMemory) {
  int block_id;
  int64_t size = 1024;
...
TEST_F(BlockManagerAscendTest, SwapInAndSwapOut) {
  // 在 device 上分配两个 block
  std::vector<int> blocks;
  Status status = block_manager->AllocateBlocks(2, blocks);
  EXPECT_TRUE(status.OK());
  ...
  std::vector<void*> addrs;
  block_manager->GetBlockPtrs(blocks, addrs);
  constexpr size_t dst_max_size = 1024ul;

  // 将数据从 host 复制到 block 中
  std::string string_a = "string_a";
  std::string string_b = "string_b";
  ACL_CHECK(aclrtMemcpy(addrs[0], dst_max_size, string_a.data(), string_a.size(), ACL_MEMCPY_HOST_TO_DEVICE));
  ACL_CHECK(aclrtMemcpy(addrs[1], dst_max_size, string_b.data(), string_b.size(), ACL_MEMCPY_HOST_TO_DEVICE));
  ...
  // 将 block 中的数据从 GPU 复制回 CPU
  ACL_CHECK(aclrtMemcpy(string_a.data(), dst_max_size, addrs[0], string_a.size(), ACL_MEMCPY_HOST_TO_DEVICE));
  ACL_CHECK(aclrtMemcpy(string_b.data(), dst_max_size, addrs[1], string_b.size(), ACL_MEMCPY_HOST_TO_DEVICE));
...
// 定义一个测试用例，继承自 BlockManagerAscendTest
TEST_F(BlockManagerAscendTest, GetFreeBlockNumber) {
  // 检查 CPU 和 GPU 的空闲内存块数量是否正确
  EXPECT_EQ(block_manager->GetHostFreeBlockNumber(), 2);
  EXPECT_EQ(block_manager->GetFreeBlockNumber(), 2);
