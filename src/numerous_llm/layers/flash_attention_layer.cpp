...
Status FlashAttentionLayer::Forward(const std::vector<Tensor>& input_tensors, std::vector<Tensor>& output_tensors) {
  ...
  // std::vector<Tensor> value_cache(output_tensors.begin() + 1 + cache_len, output_tensors.begin() + 1 + cache_len *
  // 2); flash_attention(layer_index_, input_tensors[0], out, key_cache, value_cache, max_position_embeddings_,
  // stream_);
  return Status();
  int total_tokens;
  int max_tokens;
  ...
  bool is_causal;  // init
  int rank;        // init
  AttenVarlen(input_tensors[0].GetPtr<void>(), input_tensors[1].GetPtr<void>(), input_tensors[2].GetPtr<void>(),
              output_tensors[0].GetPtr<void>(), input_tensors[3].GetPtr<void>(), total_tokens, max_tokens, batch,
              num_heads, head_size, is_causal, rank_, context_->GetComputeStreams()[rank_]);
  return Status();
}
