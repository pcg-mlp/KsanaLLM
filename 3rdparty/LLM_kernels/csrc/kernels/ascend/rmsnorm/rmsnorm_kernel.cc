...
class RmsNormKernel {
 ...
 private:
  ...
  TBuf<QuePosition::LCM> workspace_buf;
  TPipe pipe;
  RmsNormTiling tiling;
  uint32_t workspace_buffer_float_elems_num;
  float eps = 1e-6;

  // create queues for input, in this case depth is equal to buffer num
  TQue<QuePosition::VECIN, RMSNORM_BUFFER_NUM> input_queue;
  TQue<QuePosition::VECIN, RMSNORM_BUFFER_NUM> gamma_queue;
  // create queue for output, in this case depth is equal to buffer num
  TQue<QuePosition::VECOUT, RMSNORM_BUFFER_NUM> output_queue;
};
...
__aicore__ inline void RmsNormKernel<T>::Init(GM_ADDR input, GM_ADDR gamma, GM_ADDR output, GM_ADDR tiling_gm) {
  ...
  // reduce space
  workspace_buffer_float_elems_num = tiling.mainBsLengthAlign;
  if constexpr (sizeof(T) == sizeof(half)) {
    // tmp space
    workspace_buffer_float_elems_num += tiling.mainBshLength;
  }
  // fp32 tmp space
  workspace_buffer_float_elems_num += tiling.mainBshLength;

  pipe.InitBuffer(input_queue, RMSNORM_BUFFER_NUM, tiling.hLength * sizeof(T));
  pipe.InitBuffer(gamma_queue, RMSNORM_BUFFER_NUM, tiling.hLength * sizeof(T));
  pipe.InitBuffer(output_queue, RMSNORM_BUFFER_NUM, tiling.hLength * sizeof(T));
  pipe.InitBuffer(workspace_buf, workspace_buffer_float_elems_num * sizeof(float));
}
...
__aicore__ inline void RmsNormKernel<T>::RmsNormCompute() {
  ...
  LocalTensor<uint8_t> workspace_local = workspace_buf.Get<uint8_t>();
...
extern "C" __global__ __aicore__ void InvokeRmsNormFloatKernel(GM_ADDR input, GM_ADDR gamma, GM_ADDR output,
                                                               GM_ADDR tiling_gm) {
  RmsNormKernel<float> rms_norm_kernel;
  rms_norm_kernel.Init(input, gamma, output, tiling_gm);
  rms_norm_kernel.Process();
}