...
class SiluKernel {
 ...
 private:
  ...
  TPipe pipe;
  llm_kernels::ascend::SiluTilingConfig tiling;
...
template <typename DTYPE>
__aicore__ inline void SiluKernel<DTYPE>::Init(GM_ADDR input, GM_ADDR output, GM_ADDR tiling_gm) {
  ASSERT(GetBlockNum() != 0 && "block dim can not be zero!");
  auto tmp_tiling_gm = (__gm__ uint32_t*)tiling_gm;
  auto tmp_tiling = (uint32_t*)&tiling;
  for (int32_t i = 0; i < sizeof(llm_kernels::ascend::SiluTilingConfig) / sizeof(uint32_t);
...
template <typename DTYPE>
__aicore__ inline void SiluKernel<DTYPE>::Process() {
...
template <typename DTYPE>
__aicore__ inline void SiluKernel<DTYPE>::CopyIn(uint32_t loop_idx) {
...
template <typename DTYPE>
__aicore__ inline void SiluKernel<DTYPE>::CastCompute(uint32_t loop_idx) {
...
template <typename DTYPE>
__aicore__ inline void SiluKernel<DTYPE>::CopyOut(uint32_t loop_idx) {
...
extern "C" __global__ __aicore__ void InvokeSiluHalfKernel(GM_ADDR input, GM_ADDR output, GM_ADDR tiling_gm) {
  SiluKernel<half> silu_kernel;
  silu_kernel.Init(input, output, tiling_gm);
  silu_kernel.Process();
}
