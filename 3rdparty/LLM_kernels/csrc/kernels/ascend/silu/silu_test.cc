...
#include "aclrtlaunch_InvokeSiluHalfKernel.h"
#include "csrc/kernels/ascend/silu/silu.h"
...
class LlamaAscendSiluTestSuit : public AscendTestSuitBase {
 public:
 ...
 protected:
  ...
  void TestSilu() {
    ...
    for (size_t i = 0; i < total_elem_num; ++i) {
      output_ref[i] = DTYPE(float(DTYPE(input_ref[i])) * (1.0f / (1 + std::exp(-float(DTYPE(input_ref[i]))))));
    }

    SiluTilingConfig *buf = &tiling;
    size_t tiling_size = sizeof(SiluTilingConfig);
    ...
    ACL_CHECK_RET(
        ACLRT_LAUNCH_KERNEL(InvokeSiluHalfKernel)(dim_num, stream, input_device, output_device, tiling_device));
...
TEST_F(LlamaAscendSiluTestSuit, CommonTest) { TestSilu<half_float::half>(); }
