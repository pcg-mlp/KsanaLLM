...
TEST_F(LlamaAscendElementwiseTestSuit, AddTest) {
  ...
  ACL_CHECK_RET(aclrtMemcpyAsync(other_workspace, GetShapeSize(input_shape) * sizeof(half_float::half),
                                 ...
                                 ACL_MEMCPY_HOST_TO_DEVICE, stream));
  Add(input_tensor, other_tensor, add_alpha, &output_tensor, stream, llm_kernels::utils::GetTestWorkSpaceFunc);
  ...
  for (size_t i = 0; i < input_vec_host.size(); ++i) {
    EXPECT_NEAR(float(input_vec_host[i]) + float(other_vec_host[i]) * 1.0f, float(out_vec_host[i]), 1e-3);
  }
