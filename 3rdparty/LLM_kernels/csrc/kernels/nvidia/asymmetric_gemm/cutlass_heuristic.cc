...
TileShape GetCTAShapeForConfig(CutlassTileConfig tile_config) {
  switch (tile_config) {
    ...
    default:
      throw std::runtime_error("[get_grid_shape_for_config] Invalid config");
  }
}

bool IsValidSplitKFactor(const int64_t m, const int64_t n, const int64_t k, const TileShape tile_shape,
                         const int32_t split_k_factor, const size_t workspace_bytes, const bool is_weight_only) {
  // All tile sizes have a k_tile of 64.
  static constexpr int32_t k_tile = 64;
  ...
  if (is_weight_only) {
    ...
    const int32_t k_elements_per_split = k / split_k_factor;
  ...
  const int32_t ctas_in_m_dim = (m + tile_shape.m - 1) / tile_shape.m;
  const int32_t ctas_in_n_dim = (n + tile_shape.n - 1) / tile_shape.n;
  const int32_t required_ws_bytes = split_k_factor == 1 ? 0 : sizeof(int32_t) * ctas_in_m_dim * ctas_in_n_dim;
...
std::vector<CutlassTileConfig> GetCandidateTiles(const int32_t sm, const bool is_weight_only,
                                                 const bool simt_configs_only, const bool int8_configs_only) {
  ...
  if (sm >= NVIDIA_TURING_GPU_COMPUTE_CAPABILITY) {
...
std::vector<CutlassGemmConfig> GetCandidateConfigs(int32_t sm, const bool is_weight_only, const bool simt_configs_only,
                                                   const bool int8_configs_only, const int32_t max_split_k) {
  std::vector<CutlassTileConfig> tiles = GetCandidateTiles(sm, is_weight_only, simt_configs_only, int8_configs_only);

  std::vector<CutlassGemmConfig> candidate_configs;
  const int32_t min_stages = int8_configs_only ? 3 : 2;
  const int32_t max_stages = int8_configs_only ? 6 : (sm >= NVIDIA_AMPERE_GPU_COMPUTE_CAPABILITY ? 4 : 2);
  for (const auto& tile_config : tiles) {
    for (int32_t stages = min_stages; stages <= max_stages; ++stages) {
      CutlassGemmConfig config{tile_config, SplitKStyle::NO_SPLIT_K, 1, stages};
      candidate_configs.push_back(config);
      if (sm >= NVIDIA_TURING_GPU_COMPUTE_CAPABILITY) {
        for (int32_t split_k_factor = 2; split_k_factor <= max_split_k; ++split_k_factor) {
...
CutlassGemmConfig EstimateBestConfigFromOccupancies(const std::vector<CutlassGemmConfig>& candidate_configs,
                                                    const std::vector<int32_t>& occupancies, const int64_t m,
                                                    const int64_t n, const int64_t k, const int64_t num_experts,
                                                    const int32_t split_k_limit, const size_t workspace_bytes,
                                                    const int32_t multi_processor_count, const int32_t is_weight_only) {
  constexpr int32_t processor_thread_num = 256;

  if (occupancies.size() != candidate_configs.size()) {
    throw std::runtime_error(
        "[EstimateBestConfigFromOccupancies] occpancies and "
  ...
  int32_t config_waves = INT_MAX;
  int32_t current_m_tile = 0;

  const int32_t max_split_k = n >= multi_processor_count * processor_thread_num ? 1 : split_k_limit;
  for (int32_t ii = 0; ii < candidate_configs.size(); ++ii) {
    CutlassGemmConfig candidate_config = candidate_configs[ii];
    TileShape tile_shape = GetCTAShapeForConfig(candidate_config.tile_config);
    int32_t occupancy = occupancies[ii];
    ...
    for (int32_t split_k_factor = 1; split_k_factor <= max_split_k; ++split_k_factor) {
      if (IsValidSplitKFactor(m, n, k, tile_shape, split_k_factor, workspace_bytes, is_weight_only)) {
        const int32_t ctas_per_wave = occupancy * multi_processor_count;
        const int32_t ctas_for_problem = ctas_in_m_dim * ctas_in_n_dim * split_k_factor;

        const int32_t num_waves_total = (ctas_for_problem + ctas_per_wave - 1) / ctas_per_wave;
  ...
  if (best_config.tile_config == CutlassTileConfig::ChooseWithHeuristic) {
    throw std::runtime_error("Heurisitc failed to find a valid config.");
  }
