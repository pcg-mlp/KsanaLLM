    ...
    float const** alpha_scale_ptr_array, cudaStream_t stream, int* kernel_occupancy = nullptr)
{
    ...
    if (!use_fused_moe)
    {
        ...
        using EpilogueOp = typename llm_kernels::nvidia::cutlass_extensions::Epilogue<CutlassGemmOutputType,
        ...
        {
            KLLM_KERNEL_CHECK_WITH_INFO(weight_scales == nullptr && biases == nullptr && alpha_scale_ptr_array,
    ...
    int* occupancy = nullptr)
{
    ...
    {
        genericMoeGemmKernelLauncher<T, WeightType, GemmOutputType, Arch, EpilogueTag,
    ...
    float const** alpha_scale_ptr_array, cudaStream_t stream, int* occupancy = nullptr)
{
    ...
    case 4:
        dispatch<T, WeightType, GemmOutputType, arch, EpilogueTag, ThreadblockShape, WarpShape, 4>(A, B, weight_scales,
            ...
            gemm_config, multi_processor_count, use_fused_moe, alpha_scale_ptr_array, stream, occupancy);
        break;
    default: KLLM_KERNEL_THROW("dispatchGemmConfig does not support stages %d", gemm_config.stages); break;
    }
}
    ...
    float const** alpha_scale_ptr_array, cudaStream_t stream, int* occupancy = nullptr)
{
    ...
    case cutlass_extensions::CutlassTileConfig::CtaShape128x128x64_WarpShape64x32x64:
        dispatchGemmConfig<T, WeightType, GemmOutputType, arch, EpilogueTag, cutlass::gemm::GemmShape<128, 128, 64>,
            ...
            use_fused_moe, alpha_scale_ptr_array, stream, occupancy);
        break;
    case cutlass_extensions::CutlassTileConfig::Undefined: KLLM_KERNEL_THROW("GEMM config undefined."); break;
    case cutlass_extensions::CutlassTileConfig::ChooseWithHeuristic:
        ...
        break;
    default: KLLM_KERNEL_THROW("Config is invalid for same type tensorop GEMM."); break;
    }
}
    ...
    float const** alpha_scale_ptr_array, cudaStream_t stream, int* occupancy = nullptr)
{
    ...
    case cutlass_extensions::CutlassTileConfig::ChooseWithHeuristic:
        KLLM_KERNEL_THROW("GEMM config should have already been set by heuristic.");
        break;
    default: KLLM_KERNEL_THROW("Config is invalid for mixed type tensorop GEMM."); break;
    }
}
    ...
    float const** alpha_scale_ptr_array, cudaStream_t stream, int* occupancy = nullptr)
{
    ...
    case cutlass_extensions::CutlassTileConfig::ChooseWithHeuristic:
        KLLM_KERNEL_THROW("GEMM config should have already been set by heuristic.");
        break;
    default: KLLM_KERNEL_THROW("Unsupported config for float MoE gemm."); break;
    }
}
        ...
        if constexpr (isValidHopperMOESpecialisation<T, WeightType, EpilogueTag>())
        {
            ...
            if (gemm_config.is_sm90)
            {
                ...
                auto select_function = [&]()
                {
                    ...
                    default: KLLM_KERNEL_THROW("Unimplemented fusion %d requested", (int) hopper_input.fusion);
                    };
                };
    ...
    float const** alpha_scale_ptr_array, cudaStream_t stream, cutlass_extensions::CutlassGemmConfig chosen_conf)
{
    ...
    case ActivationType::Geglu:
        runGemm<cutlass_extensions::EpilogueOpDefaultFtGelu>(A, B, weight_scales, biases, bias_is_broadcast, C,
            ...
            alpha_scale_ptr_array, stream, chosen_conf);
        break;
    case ActivationType::InvalidType: KLLM_KERNEL_THROW("Activation type for fpA_intB must be valid."); break;
    default: KLLM_KERNEL_THROW("Invalid activation type."); break;
    }
}
