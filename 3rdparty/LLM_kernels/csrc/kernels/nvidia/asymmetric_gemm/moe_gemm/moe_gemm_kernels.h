...
class MoeGemmRunner {
 public:
  ...
  void InvokeMoeGemmWithBiasAct(const T* A, const WeightType* B, const T* weight_scales, const T* biases, T* C,
                                int64_t* total_rows_before_expert, int64_t total_rows, int64_t gemm_n, int64_t gemm_k,
                                int32_t num_experts, ActivationType activation_type, cudaStream_t stream);

  void InvokeMoeGemm(const T* A, const WeightType* B, const T* weight_scales, T* C, int64_t* total_rows_before_expert,
                     int64_t total_rows, int64_t gemm_n, int64_t gemm_k, int32_t num_experts, cudaStream_t stream);
 ...
 private:
  ...
  void DispatchToArch(const T* A, const WeightType* B, const T* weight_scales, const T* biases, T* C,
                      int64_t* total_rows_before_expert, int64_t total_rows, int64_t gemm_n, int64_t gemm_k,
                      int32_t num_experts, cutlass_extensions::CutlassGemmConfig gemm_config, cudaStream_t stream,
