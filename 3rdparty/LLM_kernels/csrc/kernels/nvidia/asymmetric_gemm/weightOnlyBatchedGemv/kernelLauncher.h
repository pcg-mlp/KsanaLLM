/*
 * Adapted from
 * https://github.com/NVIDIA/TensorRT-LLM/blob/v0.11.0/cpp/tensorrt_llm/kernels/weightOnlyBatchedGemv/kernelLauncher.h
 *
...
#pragma once
#include "csrc/kernels/nvidia/asymmetric_gemm/weightOnlyBatchedGemv/common.h"
#include "csrc/kernels/nvidia/asymmetric_gemm/weightOnlyBatchedGemv/details.h"
...
#define EXEC(KType, A, B, Layout, ConverterInterleave)                                                              \
  if (params.type == KType) {                                                                                       \
    select_gs<kernel_type_traits<KType>::isGroupwise, KernelDetails<A, B, Layout, ConverterInterleave, 64>>(params, \
                                                                                                            s);     \
    return;                                                                                                         \
  }
#define EXEC_W4A8(KType, A, B, Layout, ConverterInterleave)                                                          \
  if (params.type == KType && params.apply_alpha_in_advance) {                                                       \
    select_gs<kernel_type_traits<KType>::isGroupwise, KernelDetails<A, B, Layout, ConverterInterleave, 128>>(params, \
                                                                                                             s);     \
    return;                                                                                                          \
  }
  if (arch >= 70 && arch < 75) {
...
#define SUPPORT(Type) \
  if (kernel_type == Type) return true;
  if (arch >= 70 && arch < 75) {
