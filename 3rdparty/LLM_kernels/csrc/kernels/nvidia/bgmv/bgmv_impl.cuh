...
// This file is adopt
// from
// https://github.com/vllm-project/vllm/blob/891070257c145b506a20666a3cb70afcf674d4ca/csrc/punica/bgmv/bgmv_impl.cuh
...
// nthrs = (32, 4)
template <int32_t feat_in, int32_t feat_out, size_t vec_size, size_t X_copy_size, size_t W_copy_size, int32_t tx,
          int32_t ty, int32_t tz, typename in_T, typename out_T, typename W_T>
__global__ void InvokeBGMVShrinkKernel(out_T *__restrict__ Y, const in_T *__restrict__ X, const W_T *__restrict__ W,
                                       const int64_t *__restrict__ indices, int64_t y_offset, int64_t full_y_size,
  ...
  for (tile_idx = 1; tile_idx < (feat_in + tile_size - 1) / tile_size; ++tile_idx) {
    ...
    if (tile_idx * tile_size + threadIdx.y * tx * vec_size < feat_in) {
      cuda::memcpy_async(
          ...
          W + (idx * feat_out + j) * feat_in + tile_idx * tile_size + (threadIdx.y * tx + threadIdx.x) * vec_size,
          cuda::aligned_size_t<W_copy_size>(W_copy_size), pipe);
...
#pragma unroll
  ...
  y_warpwise[threadIdx.y] = ((tile_idx - 1) * tile_size + threadIdx.y * tx * vec_size < feat_in) ? sum : 0.f;
  block.sync();
#pragma unroll
  for (size_t i = 0; i < ty; ++i) {
    y += y_warpwise[i];
...
// nthrs = (2, 16, 4)
template <int32_t feat_in, int32_t feat_out, size_t vec_size, int32_t tx, int32_t ty, int32_t tz, typename in_T,
          typename out_T, typename W_T>
__global__ void InvokeBGMVExpandKernel(out_T *__restrict__ Y, const in_T *__restrict__ X, const W_T *__restrict__ W,
                                       const int64_t *__restrict__ indices, int64_t y_offset, int64_t full_y_size,
                                       int64_t num_layers, int64_t layer_idx, float scale) {
  size_t batch_idx = blockIdx.y;
  int64_t idx = indices[batch_idx] * num_layers + layer_idx;
...
void InvokeBGMVWrapper(out_T *__restrict__ Y, const in_T *__restrict__ X, const W_T *__restrict__ W,
  ...
  constexpr int32_t tz = 4;

  if constexpr (feat_in < feat_out) {
    static_assert(feat_in % vec_size == 0);
    constexpr int32_t tx = feat_in / vec_size;

    static_assert((32 % tx == 0 && feat_out % (32 / tx * tz) == 0) ||
                  (16 % tx == 0 && feat_out % (16 / tx * tz) == 0) || (8 % tx == 0 && feat_out % (8 / tx * tz) == 0));

    if constexpr (32 % tx == 0 && feat_out % (32 / tx * tz) == 0) {
      constexpr int32_t ty = 32 / tx;
      dim3 nblks(feat_out / (ty * tz), batch_size);
      dim3 nthrs(tx, ty, tz);

      InvokeBGMVExpandKernel<feat_in, feat_out, vec_size, tx, ty, tz>
          <<<nblks, nthrs, 0, stream>>>(Y, X, W, indices, y_offset, full_y_size, num_layers, layer_idx, scale);
    } else if (16 % tx == 0 && feat_out % (16 / tx * tz) == 0) {
      constexpr int32_t ty = 16 / tx;
      dim3 nblks(feat_out / (ty * tz), batch_size);
      dim3 nthrs(tx, ty, tz);
    ...
    static_assert(feat_in % (vec_size * 32) == 0 || feat_in % (vec_size * 16) == 0 || feat_in % (vec_size * 8) == 0);

    if constexpr (feat_in % (vec_size * 32) == 0) {
      constexpr int32_t tx = 32;
      constexpr int32_t ty = 4;

      dim3 nblks(feat_out, batch_size);
      dim3 nthrs(tx, ty);

      InvokeBGMVShrinkKernel<feat_in, feat_out, vec_size, vec_size * sizeof(in_T), vec_size * sizeof(W_T), tx, ty, tz>
          <<<nblks, nthrs, 0, stream>>>(Y, X, W, indices, y_offset, full_y_size, num_layers, layer_idx, scale);
      ...
      InvokeBGMVShrinkKernel<feat_in, feat_out, vec_size / 2, vec_size * sizeof(in_T) / 2, vec_size * sizeof(W_T) / 2,
                             tx, ty, tz>
          <<<nblks, nthrs, 0, stream>>>(Y, X, W, indices, y_offset, full_y_size, num_layers, layer_idx, scale);
    } else if constexpr (feat_in % (vec_size / 2 * 16) == 0) {
      constexpr int32_t tx = 16;
      constexpr int32_t ty = 4;
...
#define INST_BGMV(feat_in, feat_out, in_T, out_T, W_T)                                                \
  template void InvokeBGMVWrapper<feat_in, feat_out>(                                                 \
