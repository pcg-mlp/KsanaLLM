...
#include "csrc/kernels/nvidia/cutlass_extensions/gemm/kernel/gemm_moe_problem_visitor.h"
#include "csrc/kernels/nvidia/cutlass_extensions/tile_interleaved_layout.h"
...
struct MoeFCGemm {
 public:
  ...
  static int32_t const kAlignmentA = MapArguments::kAlignmentA;
  static int32_t const kAlignmentB = MapArguments::kAlignmentB;
  static int32_t const kAlignmentC = Epilogue::OutputTileIterator::kElementsPerAccess;
  ...
  struct Arguments {
    ...
    /// Ctor
    CUTLASS_HOST_DEVICE
    Arguments(int32_t problem_count, int32_t threadblock_count, int32_t group_size,
              typename EpilogueOutputOp::Params output_op, const ElementA* ptr_A, const ElementB* ptr_B,
              const ElementScale* weight_scales, const ElementC* ptr_C, ElementC* ptr_D,
              int64_t* total_rows_before_expert, int64_t gemm_n, int64_t gemm_k,
  ...
  struct Params {
    ...
    CUTLASS_HOST_DEVICE
    void update(Arguments const& args, void* workspace = nullptr, int32_t tile_count = 0) {
 ...
 public:
  ...
  struct KernelRunner<true, dummy> {
    ...
    static void run_kernel(Params const& params, SharedStorage& shared_storage) {
      ...
      static constexpr int32_t kInterleave = Mma::IteratorB::Shape::kRow / Mma::Shape::kK;
      ...
      while (problem_visitor.next_tile()) {
        ...
        cutlass::gemm::GemmCoord threadblock_offset(int32_t(cta_idx / grid_shape.n()) * Mma::Shape::kM,
                                                    int32_t(cta_idx % grid_shape.n()) * Mma::Shape::kN, 0);
        ...
        int32_t gemm_k_iterations = (problem_size.k() + Mma::Shape::kK - 1) / Mma::Shape::kK;
