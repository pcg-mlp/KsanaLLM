...
__global__ void topKStage1(const T* __restrict logProbs, T* tmpLogProbs, int* topKTmpIdBuf, T* topKTmpValBuf,
                           const FinishedState* finished, const int maxTopK, const int* topKs, const int vocabSize,
                           const int* endIds, const bool* skipDecode, const int* batchSlots) {
...
__global__ void topKStage2Sampling(const int* __restrict topKTmpIdBuf, T* topKTmpValBuf, int** ids,
                                   ...
                                   FinishedState* finishedOutput, float* cumLogProbs, float* outputLogProbs,
                                   const int maxTopK, const int* topKs, const float topP, const float* topPs,
                                   curandState_t* curandstate, const int* endIds, const int vocabSize,
                                   const bool* skipDecode, const int* batchSlots, int maxBatchSize,
  ...
  if (tid == 0) {
    ...
    for (int i = 0; i < k; i++) {
      ...
      if (randNum <= 0.0f || i == k - 1) {
        ...
        auto curSeqLen = 0;
        if (sequenceLengths != nullptr) {
          curSeqLen = sequenceLengths[batchSlot];
        }
        ...
        if (cumLogProbs != nullptr || outputLogProbs != nullptr) {
          ...
          if (outputLogProbs != nullptr) {
            ...
            outputLogProbs[curSeqLen * maxBatchSize + batchSlot] = normalizeLogProbs ? logProb - logf(s_sum) : logProb;
          }
        }
        break;
      }
...
#define CASE_K(K_MAX, BLOCK_SIZE_1_, BLOCK_SIZE_2_, BLOCKS_PER_BEAM_, normalizeLogProbs)                       \
  topKStage1<T, BLOCK_SIZE_1_, BLOCKS_PER_BEAM_><<<batchSize * BLOCKS_PER_BEAM_, BLOCK_SIZE_1_, 0, stream>>>(  \
      logProbs, tempLogProbs, topKTmpIdBuf, topKTmpValBuf, finishedInput, maxTopK, topKs, vocabSize, endIds,   \
      skipDecode, batchSlots);                                                                                 \
  topKStage2Sampling<T, BLOCK_SIZE_2_, BLOCKS_PER_BEAM_>                                                       \
      <<<batchSize, BLOCK_SIZE_2_, K_MAX * sizeof(int) + K_MAX * sizeof(float), stream>>>(                     \
          topKTmpIdBuf, topKTmpValBuf, ids, sequenceLengths, finishedInput, finishedOutput, cumLogProbs,       \
          outputLogProbs, maxTopK, topKs, topP, topPs, curandstate, endIds, vocabSize, skipDecode, batchSlots, \
          maxBatchSize, normalizeLogProbs, logitsHasProbs);                                                    \
  break;
...
void invokeBatchTopKSampling(void* workspace, size_t& workspaceSize, const T* logProbs, int** ids, int* sequenceLengths,
  ...
  if (workspace == nullptr) {
    workspaceSize = sizeof(T) * tempLogProbsBufSize + sizeof(int) * topKTmpIdsBufSize + sizeof(T) * topKTmpValBuf_size;
    return;
  }
  ...
  switch (logMaxTopK) {
    ...
    default:
      throw std::domain_error("top-k kernel supports 1<=k<=1024 but got k=" + std::to_string(maxTopK));
  }
}
...
template void invokeBatchTopKSampling(void* workspace, size_t& workspaceSize, const float* logProbs, int** ids,
                                      int* sequenceLengths, const FinishedState* finishedInput,
                                      FinishedState* finishedOutput, float* cumLogProbs, float* outputLogProbs,
                                      curandState_t* curandstate, const int maxTopK, const int* topKs, const float topP,
                                      const float* topPs, const int vocabSizePadded, const int* endIds,
                                      const int* batchSlots, cudaStream_t stream, const int batchSize, int maxBatchSize,
                                      const bool* skipDecode, const bool normalizeLogProbs, const bool logitsHasProbs);
...
void invokeTopKSampling(void* workspace, size_t& workspaceSize, const T* logProbs, int** ids, int* sequenceLengths,
  ...
  invokeBatchTopKSampling(workspace, workspaceSize, logProbs, ids, sequenceLengths, finishedInput, finishedOutput,
                          cumLogProbs, outputLogProbs, curandstate, topK, nullptr, topP, nullptr, vocabSizePadded,
                          endIds, batchSlots, stream, batchSize, maxBatchSize, skipDecode, normalizeLogProbs,
                          logitsHasProbs);
}
...
template void invokeTopKSampling(void* workspace, size_t& workspaceSize, const float* logProbs, int** ids,
                                 int* sequenceLengths, const FinishedState* finishedInput,
                                 FinishedState* finishedOutput, float* cumLogProbs, float* outputLogProbs,
                                 curandState_t* curandstate, const int topK, const float topP,
                                 const int vocabSizePadded, const int* endIds, const int* batchSlots,
                                 cudaStream_t stream, const int batchSize, int maxBatchSize, const bool* skipDecode,
