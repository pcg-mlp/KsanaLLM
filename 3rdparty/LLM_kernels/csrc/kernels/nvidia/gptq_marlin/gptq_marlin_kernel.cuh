...
template <typename scalar_t, scalar_type::ScalarTypeId w_type_id>
...
//
template <>
__device__ inline typename ScalarType<half>::FragB dequant<half, scalar_type::kU4B8.id()>(int q) {
...
template <>
__device__ inline typename ScalarType<nv_bfloat16>::FragB dequant<nv_bfloat16, scalar_type::kU4B8.id()>(int q) {
  ...
  frag_b[0] = __hfma2(*reinterpret_cast<nv_bfloat162*>(&lo), *reinterpret_cast<const nv_bfloat162*>(&MUL),
                      *reinterpret_cast<const nv_bfloat162*>(&ADD));
  frag_b[1] = __hfma2(*reinterpret_cast<nv_bfloat162*>(&hi), *reinterpret_cast<const nv_bfloat162*>(&MUL),
                      *reinterpret_cast<const nv_bfloat162*>(&ADD));
  return frag_b;
}

template <>
__device__ inline typename ScalarType<half>::FragB dequant<half, scalar_type::kU4.id()>(int q) {
  const int LO = 0x000f000f;
  const int HI = 0x00f000f0;
  const int EX = 0x64006400;
  // Guarantee that the `(a & b) | c` operations are LOP3s.
...
template <>
__device__ inline typename ScalarType<nv_bfloat16>::FragB dequant<nv_bfloat16, scalar_type::kU4.id()>(int q) {
  static constexpr uint32_t MASK = 0x000f000f;
  static constexpr uint32_t EX = 0x43004300;
...
//
template <>
__device__ inline typename ScalarType<half>::FragB dequant<half, scalar_type::kU8B128.id()>(int q) {
...
template <>
__device__ inline typename ScalarType<nv_bfloat16>::FragB dequant<nv_bfloat16, scalar_type::kU8B128.id()>(int q) {
  typename ScalarType<nv_bfloat16>::FragB frag_b;
...
template <>
__device__ inline typename ScalarType<half>::FragB dequant<half, scalar_type::kU8.id()>(int q) {
  static constexpr uint32_t mask_for_elt_01 = 0x5250;
  static constexpr uint32_t mask_for_elt_23 = 0x5351;
  static constexpr uint32_t start_byte_for_fp16 = 0x64646464;
...
template <>
__device__ inline typename ScalarType<nv_bfloat16>::FragB dequant<nv_bfloat16, scalar_type::kU8.id()>(int q) {
  typename ScalarType<nv_bfloat16>::FragB frag_b;
...
template <typename scalar_t,                          // compute dtype, half or nv_float16
          const scalar_type::ScalarTypeId w_type_id,  // weight ScalarType id
...
__global__ void Marlin(const int4* __restrict__ A,           // fp16 input matrix of shape mxk
  ...
  static constexpr auto w_type = scalar_type::ScalarType::from_id(w_type_id);
...
#pragma unroll
    for (int j = 0; j < 4; j++) {
      ...
      if constexpr (has_act_order) {
        scale4<scalar_t>(frag_b0, act_frag_s[k % 2][0][j], act_frag_s[k % 2][1][j], act_frag_s[k % 2][2][j],
      ...
      if constexpr (has_act_order) {
        scale4<scalar_t>(frag_b1, act_frag_s[k % 2][0][j], act_frag_s[k % 2][1][j], act_frag_s[k % 2][2][j],
...
#pragma unroll
            for (int j = 0; j < 2 * 4; j++) {
              reinterpret_cast<scalar_t*>(&c)[j] =
                  Dtype::float2num(reinterpret_cast<float*>(&frag_c)[4 * 2 * 4 * (i / 4) + 4 * j + (i % 4)]);
            }
...
#pragma unroll
      for (int k = 0; k < th_size; k++) {
        sh[threadIdx.x] = C_tmp[c_cur_offset + active_threads * k + threadIdx.x];
...
#pragma unroll
            for (int j = 0; j < 4; j++) {
              ...
              scale_float<scalar_t>(reinterpret_cast<float*>(&frag_c[i][j][0][2]), frag_s[j / 2][2 * (j % 2) + 0]);
