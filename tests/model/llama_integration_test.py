...
if __name__ == "__main__":
    ...
    npu_ref_result = [
        "如果我是国际空间站上的宇航员，目睹了外星实体接近空间站，我会立即向地面控制团队发出紧急通信。我会使用空间站上的通信设备，"
        "向地面控制团队报告我的观察结果和建议。首先，我会向地面控制团队报告我所看到的外星实体的位置、形状、大小和颜色等详细信息。"
        "我会尽可能提供准确的描述，以便地面控制团队能够更好地了解外星实体的情况。其次，我会向地面控制团队报告外星实体的行动轨迹和"
        "速度。我会尽可能提供详细的信息，以便地面控制团队能够更好地了解外星实体的行动方式。最后，我会向地面控制团队提出建议。我会"
        "建议地面控制团队采取适当的措施，以确保空间站和宇航员的安全。我会建议地面控制团队立即启动应急计划，并与国际空间站上的其他"
        "宇航员和地面控制团队成员保持联系。总之，如果我是国际空间站上的宇航员，目睹了外星实体接近空间站，我会立即向地面控制团队发"
        "出紧急通信，并提供尽可能详细的观察结果和建议，以确保空间站和宇航员的安全。</s>",
        "作为一名国际空间站上的宇航员，如果我意外地目睹了外星实体接近空间站，我的第一反应可能是惊恐和惊讶。但是，我们的训练和准备"
        "使我们能够在任何情况下保持冷静和专业。在太空中吃饭是一项非常重要的任务，因为我们需要保持身体健康和精神状态良好，以便完成"
        "我们的任务。在太空中，我们使用特殊的食品和餐具，以确保我们的食物和饮料不会漂浮到空间站的其他部分或飘出太空舱。我们的食物"
        "通常是预包装的，并且在太空中使用特殊的加热器和冷却器进行加热和冷却。我们还使用特殊的餐具，例如吸管和勺子，以避免食物和液"
    ...
    ref_result = npu_ref_result if is_npu else gpu_ref_result
    ...
    if not is_npu:
        # NOTE(karlluo): there is not prefix cache with npu mode
        result = infer(none_prefix_prompt[0], tokenizer, generation_config,
                       model)
